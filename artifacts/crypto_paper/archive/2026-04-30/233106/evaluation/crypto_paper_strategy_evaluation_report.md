# Crypto Paper Strategy Evaluation

Paper-only evaluation. Metrics are based on simulated fills, fees, and slippage.

## Executive Summary
- Closed trades: 0
- Open trades: 3
- Net profit: 0.000000
- Win rate: 0.0000
- Profit factor: n/a
- Expectancy: n/a

## Trade Quality
- Average win: n/a
- Average loss: n/a
- Average win/loss ratio: n/a
- Best trade: None
- Worst trade: None
- Consecutive wins max: 0
- Consecutive losses max: 0

## Exit Breakdown
- STOP_LOSS: count=0 total_net_pnl=0
- TAKE_PROFIT: count=0 total_net_pnl=0
- MANUAL_SELL: count=0 total_net_pnl=0
- UNKNOWN: count=0 total_net_pnl=0

## Fees and Slippage
- Total fees: 0.000000
- Total slippage: 0.000000
- Fee drag % of gross P&L: None

## Per-Symbol Results
| Symbol | Trades | Win rate | Net P&L | Expectancy | Fees | Stop-loss count | Take-profit count |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| BTCUSDT | 0 | 0.0000 | 0.000000 | None | 0.000000 | 0 | 0 |

## Open Trades
- BTCUSDT: qty=0.00032883 entry_price=76026.524265 entry_time=2026-04-28T15:00:08.211431 unrealized=0.021594776970420292
- BTCUSDT: qty=0.00032711 entry_price=76427.47464 entry_time=2026-04-28T20:00:10.214557 unrealized=0.021481487424152863
- BTCUSDT: qty=0.00032720 entry_price=76405.993905 entry_time=2026-04-28T22:00:09.418034 unrealized=0.02148752671655887

## Warnings
- Small sample size: fewer than 30 closed trades.
- No closed trades available; strategy metrics are limited.
- Open trades are excluded from closed-trade expectancy.
- Paper-only results; no real execution occurred.
- Fees and slippage are simulated.

## Notes
- Paper-only.
- No live orders placed.
- No broker integration.
- Metrics are based on simulated fills, fees, and slippage.
