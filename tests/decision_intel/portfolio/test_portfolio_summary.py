...
class PortfolioSummaryTests(unittest.TestCase):
    def test_portfolio_summary_from_aggregation(self):
        with TemporaryDirectory() as tmp:
            ...
            summary_path, entry = summarize_portfolio(run_id=run_id, base_path=str(base))
            self.assertTrue(summary_path.exists())
            payload = json.loads(summary_path.read_text(encoding="utf-8"))
            self.assertEqual(payload["number_of_assets"], 2)
            self.assertEqual(payload["total_weight"], 1.0)
            self.assertAlmostEqual(payload["total_weighted_signal"], 0.56, places=12)
            self.assertEqual(payload["count_missing_weights"], 0)
            self.assertEqual(entry["name"], "portfolio.summary")
            self.assertTrue(entry["path"].startswith("artifacts/portfolio/"))

            summarize_portfolio(run_id=run_id, base_path=str(base))
            updated = json.loads(manifest_path.read_text(encoding="utf-8"))
            entries = [e for e in updated["artifact_index"] if e["name"] == "portfolio.summary"]
