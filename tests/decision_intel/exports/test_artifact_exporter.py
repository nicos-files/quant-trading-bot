...
class ArtifactExporterTests(unittest.TestCase):
    def test_export_formats_paths_and_manifest_idempotency(self):
        with TemporaryDirectory() as tmp:
            ...
            export_artifacts(run_id=run_id, base_path=str(base))
            updated = json.loads(manifest_path.read_text(encoding="utf-8"))
            export_entries = [e for e in updated["artifact_index"] if e["name"].startswith("export.")]
            self.assertEqual(len(export_entries), 2)
            formats = {e["type"] for e in export_entries}
            self.assertTrue(formats.issubset({"export.parquet", "export.csv"}))
            for entry in export_entries:
                self.assertFalse(Path(entry["path"]).is_absolute())
                self.assertTrue((run_root / entry["path"]).exists())
                if entry["type"] == "export.parquet":
                    self.assertTrue(entry["path"].endswith(".parquet"))
                if entry["type"] == "export.csv":
                    self.assertTrue(entry["path"].endswith(".csv"))

            export_artifacts(run_id=run_id, base_path=str(base))
            updated_again = json.loads(manifest_path.read_text(encoding="utf-8"))
            export_entries_again = [e for e in updated_again["artifact_index"] if e["name"].startswith("export.")]
            entries = {(e["name"], e["type"], e["path"]) for e in export_entries}
            entries_again = {(e["name"], e["type"], e["path"]) for e in export_entries_again}
