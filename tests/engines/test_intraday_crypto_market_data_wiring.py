...
class IntradayCryptoMarketDataWiringTests(unittest.TestCase):
    ...
    def test_enabled_symbols_with_strategy_disabled_is_noop_with_diagnostics(self) -> None:
        ...
        self.assertTrue(any("strategy disabled" in warning for warning in result.diagnostics.warnings))
    ...
    def test_unhealthy_provider_does_not_crash(self) -> None:
        ...
        result = engine.run(
            EngineContext(
                ...
                mode="test",
                universe=["BTCUSDT"],
                config={"crypto_universe": [{"symbol": "BTCUSDT", "enabled": True, "strategy_enabled": False}]},
    ...
    def test_strategy_enabled_but_not_implemented_is_safe_noop(self) -> None:
        engine = IntradayCryptoEngine()
        result = engine.run(
            EngineContext(
                as_of=datetime(2026, 4, 21, 12, 0, 0),
                ...
                mode="test",
                universe=["BTCUSDT"],
                config={"crypto_universe": [{"symbol": "BTCUSDT", "enabled": True, "strategy_enabled": True}]},
            )
        )
        self.assertTrue(any("not implemented" in warning for warning in result.diagnostics.warnings))
